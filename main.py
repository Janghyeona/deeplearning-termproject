...
from src.utils import square_euclidean_metric, count_acc, Averager, csv_write
...
def Test_phase(model, args, k):
    ...
    for i in range(TEST_SIZE):
        for episode in test_loader:
            ...
            z_shot = model(data_shot)
            z_dim = z_shot.size(-1)
            z_proto = z_shot.view(n_class, n_support, z_dim).mean(1)
            zq = model(data_query)
            logit = square_euclidean_metric(zq, z_proto)
...
def train(args):
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            labels = labels.view([n_class, n_query, -1])
            ...
            z_proto = None; logit = None; loss = None
